...
{
    public class DungeonRoomMapSprite : ISprite
    {
        ...
        int frameIndex;
        ITile owner;
        public DungeonRoomMapSprite(ITile par) {
            
            for (int i = 0; i < SpriteFrameCount; i++)
            {
                Frames[i] = new Rectangle(  UISpriteConstants.DungeonRoomSpritesX,
                                            UISpriteConstants.DungeonRoomSpritesY, 
                                            UISpriteConstants.DungeonRoomSpriteSize, 
                                            UISpriteConstants.DungeonRoomSpriteSize);
            }
        ...
        public void Draw() {
            destination = new Rectangle(
                                            owner.PosX + (UISpriteConstants.DungeonRoomSpriteSize * GeneralConstants.ImageScale)/2,
                                            owner.PosY + (UISpriteConstants.DungeonRoomSpriteSize * GeneralConstants.ImageScale) / 2,
                                            UISpriteConstants.DungeonRoomSpriteSize * GeneralConstants.ImageScale,
                                            UISpriteConstants.DungeonRoomSpriteSize * GeneralConstants.ImageScale);
