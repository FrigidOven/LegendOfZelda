...
namespace ZeldaNES.Sprites.Classes.WeaponSprites.Termination
{
    public class ArrowTerminationSprite : ISprite
    {
        ...
        public ArrowTerminationSprite(IWeapon weapon)
        {
            frames = new Rectangle[WeaponSpriteConstants.ArrowTerminationSpritesFrameCount];
            for (int i = 0; i < frames.Length; i++)
            {
                frames[i] = new Rectangle(WeaponSpriteConstants.TerminationSpritesIndex,
                                          WeaponSpriteConstants.ArrowTerminationSpritesIndex + i * WeaponSpriteConstants.SpriteSize,
        ...
        public void Update()
        {
            if (frameIndex == frames.Length - 1 && stallFrameCounter == WeaponSpriteConstants.ArrowTerminationSpritesStallFrameCount)
            {
                weapon.ShouldDelete = true;
            }
            if (stallFrameCounter == WeaponSpriteConstants.ArrowTerminationSpritesStallFrameCount)
            {
                stallFrameCounter = 0;
